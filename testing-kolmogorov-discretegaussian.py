...
# main function: sample_dgauss(sigma2)
...
def test_kolmogorov_dist(N, sigma2, with_plots=False):
    ...
    x = [sample_dgauss(sigma2) for i in range(N)] # Draw the N samples from our discrete Gaussian sampler
...
test_kolmogorov_dist(2000,10,True);
...
distances = [test_kolmogorov_dist(int(N),20) for N in np.logspace(1.0, 7.0, num=50)];
